...
private let countiesAndContinents: [(ISO3166.Continent, ISO3166.Country)] = [
    (.as, .afg),
    (.eu, .alb),
    (.an, .ata),
    (.af, .dza),
...
extension ISO3166.Continent {
    init?(county: ISO3166.Country) {
        let c = countiesAndContinents.first { $0.1 == county }
        guard let (continent, _) = c  else {
            return nil
        }
        self = continent
    }
...
extension ISO3166.Continent: CustomStringConvertible {
    fileprivate var localizedKey: String {
        switch (self) {
    ...
    var description: String {
        return NSLocalizedString(localizedKey, tableName: nil, bundle: Bundle(for: Continent.self), value: localizedKey, comment: "")
    }
}
...
extension ISO3166.Country: LocalizedStringConvertible {
    var localizeDescription: String {
        ...
        return (loc as NSLocale).displayName(forKey: NSLocale.Key.countryCode, value: threeLetterName) ?? NSLocalizedString("country.Unknown", tableName: nil, bundle: Bundle(for: Country.self), value: "country.Unknown", comment: "")
    }
}
